...
General-purpose expert-level Python coding agent for the jxa-lib project.
...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'jxa_lib': {}})
  ```

  Additional third-party loggers can be added as needed:

  ```python
  setup_logging(debug=debug, loggers={'jxa_lib': {}, 'urllib3': {}, 'soupsieve': {}})
  ```
...
- `` :py:mod:`jxa_lib.string` `` for modules
- `` :py:func:`jxa_lib.string.slugify` `` for functions
- `` :py:class:`jxa_lib.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name: `` :py:func:`~jxa_lib.string.slugify` `` renders as
  `slugify`
