...
Fixes mypy type errors and eliminates `Any` usage in the jxa-lib project.
...
1. Fix all mypy errors reported by `uv run mypy jxa_lib`
...
Place new TypedDict classes in `jxa_lib/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`jxa_lib/typing.py` when shared.
...
1. Run `uv run mypy jxa_lib` and capture all errors.
...
4. Run `uv run mypy jxa_lib` again to verify fixes.
